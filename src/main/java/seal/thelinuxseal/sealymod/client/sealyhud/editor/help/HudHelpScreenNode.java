...
import seal.thelinuxseal.sealymod.client.sealyhud.docs.HudContextScanner.FuncRecord;
import seal.thelinuxseal.sealymod.client.sealyhud.docs.HudContextScanner.ClassRecord;
import seal.thelinuxseal.sealymod.client.sealyhud.editor.help.HudHelpScreen.Position;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;

public class HudHelpScreenNode {
    private final List<FuncRecord> func = new ArrayList<>();
    private final List<HudHelpScreenNode> children = new ArrayList<>();
    private final ClassRecord me;
    private final HudHelpScreen parent;
    private boolean isExpanded = false;
    private Button expandBtn;
    public HudHelpScreenNode(Object o, HudHelpScreen parent, ClassRecord me){
        this.parent = parent;
        this.me = me;
        Component expandedStr = SealyModClient.lang.getAsComponent("sealymod.sealyhud.editor.help.node.expanded");
        Component closedStr = SealyModClient.lang.getAsComponent("sealymod.sealyhud.editor.help.node.closed");
    ...
    public void addChild(HudHelpScreenNode node){
        children.add(node);
    }
       ...
       public Position render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta, Position oldPos){
        ...
        if (isExpanded) {
            ...
            for (int i = 0; i < children.size(); i++) {
                HudHelpScreenNode child = children.get(i);
                pos2 = pos;
